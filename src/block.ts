...
export class Block {
  ...
  async validate(peer: Peer) {
    ...
    if (blockManager.deferredValidations[this.blockid] !== undefined) {
      ...
      const result = (await blockManager.deferredValidations[this.blockid].promise) as [
        boolean,
        ErrorChoice
      ];
      if (!result[0]) {
    ...
    const deferred = (blockManager.deferredValidations[this.blockid] = new Deferred<
      [boolean, string]
    >());
    ...
    try {
      ...
      if (this.isGenesis()) {
        ...
        if (!util.isDeepStrictEqual(this.toNetworkObject(), GENESIS)) {
          throw new AnnotatedError(
            "INVALID_GENESIS",
            `Invalid genesis block ${this.blockid}: ${JSON.stringify(this.toNetworkObject())}`
          );
        }
