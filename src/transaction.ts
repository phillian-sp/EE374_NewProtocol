    ...
    const inputValues = await Promise.all(
      this.inputs.map(async (input, i) => {
        ...
        if (blockCoinbase !== undefined && input.outpoint.txid === blockCoinbase.txid) {
          logger.debug(
            `Transaction ${this.txid} is spending coinbase transaction ${blockCoinbase.txid}`
          );
          throw new AnnotatedError(
            "INVALID_TX_OUTPOINT",
        ...
        for (let j = 0; j < i; j++) {
          if (
            this.inputs[j].outpoint.txid === input.outpoint.txid &&
            this.inputs[j].outpoint.index === input.outpoint.index
          ) {
            logger.debug(
              `Transaction ${this.txid} has multiple inputs that spend the same outpoint.`
            );
            throw new AnnotatedError(
              "INVALID_TX_OUTPOINT",
              `Transaction ${this.txid} has multiple inputs that spend the same outpoint.`
